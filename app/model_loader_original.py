...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        out = mdl.generate(
            **inputs,
            max_new_tokens=cfg.get("max_new_tokens", 64),   # Limit length of response
            do_sample=True,                                 # Enable randomness
            temperature=cfg.get("temperature, 0.7")         # Control creativity
        )

    # Decode generated tokens into text
    text = tok.decode(out[0], skip_special_tookens=True)

    # Display model outputs (trimmed if very long)
    print("\n [MODEL OUTPUT]")
    print(text[:300])
